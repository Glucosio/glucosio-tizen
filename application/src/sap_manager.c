...
#define TAG "GLUCOSIO.SAP_MANAGER"

#define HELLO_ACCESSORY_PROFILE_ID "glucosio_companion_gear"
...
static void
on_service_connection_terminated(sap_peer_agent_h peer_agent,
                                             sap_socket_h socket,
              ...
              void *user_data)
{
   ...
   /* TODO add necessary procedures on data receiving */
}
...
static gboolean _create_service_connection(gpointer user_data)
{
   ...
   if (result == SAP_RESULT_SUCCESS) {
      ...
      /* TODO: Connection Establishment Failed */
...
gboolean sap_request_service_connection()
{
...
static gboolean _find_peer_agent(gpointer user_data)
{
   ...
   result = sap_agent_find_peer_agent(priv->agent, on_peer_agent_updated, priv);
...
{
   priv_data.peer_callback = callback;
   priv_data.peer_callback_data = data;
