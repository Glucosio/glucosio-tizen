...
static gboolean _find_peer_agent(gpointer user_data)
{
	...
	if (result == SAP_RESULT_SUCCESS) {
		...
		dlog_print(DLOG_ERROR, LOG_TAG, "findsap_peer_agent_s is failed (%d)", result);
	}
...
gboolean send_data(char *message)
{
	int result;
	if (priv_data.socket) {
		dlog_print(DLOG_INFO, LOG_TAG, "Sending data ");
...
void initialize_sap()
{
	...
	if (agent == NULL)
		dlog_print(DLOG_INFO, LOG_TAG, "ERROR in creating agent");
	else
		dlog_print(DLOG_ERROR, LOG_TAG, "SUCCESSFULLY create sap agent");
